...
\centering
	...
	\end{tabular}
	\centeredcaption{Amount of optimally and suboptimally solved
	instances, and unsolved instances. Also, the total execution time is given for the
	CP solver in two different CPUs.}
...
In table \ref{table:benchmark:LP-results:suboptimalinstances} we can see that using more
...
\centering
	...
	\end{tabular}
	\centeredcaption{Amount of optimally and suboptimally solved
	instances, and unsolved instances. Also, the total execution time is given for the SAT framework.}
	\label{table:benchmark:SAT-results}
\end{table}

The only machine not powerful enough to solve (presumably) all instances optimally
...
\centering
	...
	\end{tabular}
	\centeredcaption{Without enforcement (left) and with enforcement (right). Time in seconds that
	the framework in figure \ref{fig:satisfiability:framework} needed to solve the different instances
	for the different encodings of the ``at most one'' constraint. The columns refer to the names
	given to the different encodings formalised in equations \ref{eq:cnf:at-most-one:quad},
	\ref{eq:cnf:at-most-one:log} and \ref{eq:cnf:at-most-one:heule}, respectively.}
