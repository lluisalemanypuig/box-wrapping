...
Although the variables needed may depend on the paradigm used to tackle the problem,
in this section are described both the variable selection made (see section
\ref{sec:modelling:variables}) and the constraints used to model the problem (see section
\ref{sec:modelling:constraints}) in a general enough way. Therefore, their use in the
implementation of the algorithms may make them to change in some way so that they can
be adapted properly to the paradigm.
...
\begin{enumerate}
    ...
    $1 \le b \le N$, $1 \le x \le W$, $1 \le y \le L$.
...
Therefore, the model consists of, at least, two 3-dimensional matrices, each containing
$N \times W \times L$ Boolean variables, and one array of $N$ Boolean variables, giving
us a total of at least $N(2WL + 1)$ Boolean variables. The actual implementation of the solvers
...
\begin{enumerate}
	...
	box is not/is rotated'' after the ``for all rectangular boxes''. Since no
    square box needs to be rotated then the rotation is fixed to a null value ($R_b = 0$).
    ...
    the roll because it would end up out of its bounds (see figure \ref{fig:box-out-bounds}).
    This will stop the solvers from assigning positive values to these cells, that is, assign
    a box, in the hope that they detect the unfeasibility of an instance as soon as possible.
    Again, the constraints for square boxes are different from those for rectangular boxes.
    Moreover, it suffices to forbid the placement of a box's top-left corner in these cells.
